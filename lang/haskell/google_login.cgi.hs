...
cgiMain = do
  ...
  code' <- getInput "code"
  case code' of
    Nothing -> output $ script
    Just code -> do
      let p = (params code appId appSecret myUrl)
      liftIO $ httpPost tokenUrl p (headers p)
      output $ code ++ script
...
params code addId appSecret myUrl
  = "code=" ++ code ++
    "&client_id=" ++ appId ++
    "&client_secret=" ++ appSecret ++
    "&redirect_uri=" ++ myUrl ++ "&grant_type=authorization_code"

headers :: String -> [Header]
headers body = [ mkHeader HdrContentType "application/x-www-form-urlencoded", mkHeader HdrContentLength (show $ Prelude.length body) ]

httpPost :: String -> String -> [Header] ->IO ()
httpPost url body headers = do
  res <- simpleHTTP req
  case res of
    Left x -> return ()
    Right r -> return ()
